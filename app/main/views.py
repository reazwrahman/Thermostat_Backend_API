...
def state():
    ...
    current_state["on_for_minutes"] = utility.get_time_delta_from_hour_minutes(
        current_state[SharedDataColumns.LAST_TURNED_ON.value]
    )
    current_state["off_for_minutes"] = utility.get_time_delta_from_hour_minutes(
        current_state[SharedDataColumns.LAST_TURNED_OFF.value]
    )
...
def deviceConfigs():
    configs = {}
    configs["maximum_on_time"] = db_api.read_column(
        SharedDataColumns.MAXIMUM_ON_TIME.value
    )
    configs["minimum_on_time"] = db_api.read_column(
        SharedDataColumns.MINIMUM_ON_TIME.value
    )
    configs["cool_down_period"] = db_api.read_column(
