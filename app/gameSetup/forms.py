...
class ActiveGamesForm(FlaskForm):
    game_selection = SelectField("Select a game: ", coerce=int)
...
class AddScoreCardForm(FlaskForm):
    score_card_link = StringField(
        "Enter the link for score card", validators=[DataRequired()]
    )
    points_per_run = FloatField(
        "Enter the weight for each run", validators=[DataRequired()]
    )
    points_per_wicket = FloatField(
        "Enter the weight for each wicket", validators=[DataRequired()]
    )
    submit = SubmitField("Setup")
